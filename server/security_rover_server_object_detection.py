...
# Ensure rover camera frames are decoded into base.latest_frame for laptop-side detection.
if str(getattr(base, "DETECTOR_BACKEND", "none")).strip().lower() == "none":
    base.DETECTOR_BACKEND = "windows_object_detection"
...
OBJECT_REVIEW_PROVIDER = (
    os.getenv("STASIS_OBJECT_REVIEW_PROVIDER", "fallback").strip().lower()
)
OBJECT_REVIEW_TIMEOUT = float(os.getenv("STASIS_OBJECT_REVIEW_TIMEOUT", "20"))
OBJECT_ANALYSIS_INTERVAL = float(
    os.getenv("STASIS_OBJECT_ANALYSIS_INTERVAL_SECONDS", "0.2")
)
ONNX_MODEL_PATH = Path(os.getenv("STASIS_YOLO_ONNX_MODEL", str(DEFAULT_MODEL_PATH)))
CUSTOM_ONNX_MODEL_PATH = Path(
    os.getenv("STASIS_CUSTOM_YOLO_ONNX_MODEL", str(DEFAULT_CUSTOM_MODEL_PATH))
)
CUSTOM_LABELS_PATH = Path(
    os.getenv("STASIS_CUSTOM_YOLO_LABELS", str(DEFAULT_CUSTOM_LABELS_PATH))
)
ONNX_INPUT_SIZE = int(os.getenv("STASIS_YOLO_INPUT_SIZE", "640"))
CUSTOM_ONNX_INPUT_SIZE = int(os.getenv("STASIS_CUSTOM_YOLO_INPUT_SIZE", "416"))
...
DRAW_OVERLAY = os.getenv("STASIS_YOLO_DRAW_OVERLAY", "true").strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
...
WINDOWS_DETECTOR_RUNTIME = (
    os.getenv("STASIS_WINDOWS_DETECTOR_RUNTIME", "ultralytics").strip().lower()
)
ULTRALYTICS_MODEL_NAME = os.getenv(
...
GREEN_STRIP_MIN_CONFIDENCE = float(
    os.getenv("STASIS_GREEN_STRIP_MIN_CONFIDENCE", "0.70")
)
GREEN_STRIP_MIN_CENTER_Y_RATIO = float(
    os.getenv("STASIS_GREEN_STRIP_MIN_CENTER_Y_RATIO", "0.45")
)
...
CUSTOM_MIN_OBJECT_CONFIDENCE = float(
    os.getenv("STASIS_CUSTOM_MIN_OBJECT_CONFIDENCE", "0.68")
)
CUSTOM_MAX_BOX_WIDTH_RATIO = float(
    os.getenv("STASIS_CUSTOM_MAX_BOX_WIDTH_RATIO", "0.58")
)
CUSTOM_MAX_BOX_HEIGHT_RATIO = float(
    os.getenv("STASIS_CUSTOM_MAX_BOX_HEIGHT_RATIO", "0.58")
)
COCO_MIN_ANIMAL_CONFIDENCE = float(
    os.getenv("STASIS_COCO_MIN_ANIMAL_CONFIDENCE", "0.68")
)
COCO_MIN_OBJECT_CONFIDENCE = float(
    os.getenv("STASIS_COCO_MIN_OBJECT_CONFIDENCE", "0.70")
)
...
DETECTION_FILTER_CONFIG_PATH = Path(
    os.getenv(
        "STASIS_DETECTION_FILTER_CONFIG",
        str(SERVER_DIR / "detection_filter_config.json"),
    )
)

detectors: list[Any] = []
detection_postprocessor = DetectionPostProcessor(
    load_detection_filter_config(DETECTION_FILTER_CONFIG_PATH)
)
...
def letterbox(frame: np.ndarray, size: int) -> tuple[np.ndarray, float, float, float]:
    ...
    canvas[
        int(pad_y) : int(pad_y) + resized_height,
        int(pad_x) : int(pad_x) + resized_width,
    ] = resized
...
class YoloOnnxDetector:
    def __init__(
        ...
        self.name = name
        self.allowed_labels = (
            {label_key(label) for label in allowed_labels} if allowed_labels else set()
        )
    ...
    def setup(self) -> bool:
        ...
        # Auto-detect input size from model shape if possible
        try:
            ...
            if len(input_shape) == 4:
                ...
                if isinstance(height_dim, int) and height_dim > 0:
                    logging.info(
                        "Windows YOLO ONNX detector %s auto-detected input size from model shape: %d",
                        self.name,
                        height_dim,
                    )
                    self.input_size = height_dim
        except Exception as e:
            logging.warning(
                "Windows YOLO ONNX detector %s failed to parse input shape: %s",
                self.name,
                e,
            )

        # Auto-detect labels from model metadata if possible
        try:
            meta = self.session.get_modelmeta()
            if (
                meta
                and meta.custom_metadata_map
                and "names" in meta.custom_metadata_map
            ):
                import ast

                names_str = meta.custom_metadata_map["names"]
                labels_dict = ast.literal_eval(names_str)
                if isinstance(labels_dict, dict):
                    detected_labels = [
                        label_key(labels_dict[i]) for i in sorted(labels_dict.keys())
                    ]
                    if detected_labels:
                        logging.info(
                            "Windows YOLO ONNX detector %s auto-detected labels from model metadata: %s",
                            self.name,
                            detected_labels,
                        )
                        self.labels = detected_labels
        except Exception as e:
            logging.warning(
                "Windows YOLO ONNX detector %s failed to parse labels from metadata: %s",
                self.name,
                e,
            )

        logging.info(
            "Windows YOLO ONNX detector %s loaded from %s with %d labels.",
            self.name,
            self.model_path,
            len(self.labels),
        )
    ...
    def detect(self, frame: np.ndarray) -> list[dict[str, Any]]:
        ...
        return self.decode_outputs(
            outputs, frame.shape[1], frame.shape[0], scale, pad_x, pad_y
        )
...
class UltralyticsDetector:
    def __init__(
        ...
        self.allowed_labels = set(self.target_classes)
    ...
    def setup(self) -> bool:
        try:
            ...
            logging.error(
                "ultralytics and torch are required for Windows GPU detector runtime."
            )
            return False
        ...
        is_yoloe = "yoloe" in model_source.lower()
        if is_yoloe and YOLOE is not None:
            self.model = YOLOE(model_source)
            self.name = "yoloe"
        else:
            self.model = YOLO(model_source)
            self.name = "yolo"

        if ULTRALYTICS_DEVICE.startswith("cuda"):
            ...
            if not cuda_ok:
                self.device = "cpu"
                logging.warning(
                    "CUDA unavailable; falling back to CPU for ultralytics detection."
                )
            else:
                try:
    ...
    def detect(self, frame: np.ndarray) -> list[dict[str, Any]]:
        ...
        try:
            results = self.model.predict(
                ...
                iou=ONNX_NMS_THRESHOLD,
                device=self.device,
                verbose=False,
            )
        except Exception as exc:
            logging.warning("Ultralytics inference failed: %s", exc)
            return []
        ...
        detections.sort(key=confidence_score, reverse=True)
        return detections[:ONNX_MAX_DETECTIONS]
...
def emit_object_detections_for_dashboard(detections: list[dict[str, Any]]) -> None:
    payload: list[dict[str, Any]] = []
    for item in detections[:ONNX_MAX_DETECTIONS]:
        ...
        payload.append(
            {
                ...
                "category": str(item.get("category_hint") or "object"),
                "confidence": float(item.get("confidence", 0) or 0),
                "bbox": [x, y, x + width, y + height],
            }
        )
    base.socketio.emit("object_detections", {"detections": payload})
    ...
    for item in detections:
        ...
        if detector == "stasis_custom" and category in {"object", "marker"}:
            ...
            if area_ratio > CUSTOM_MAX_BOX_AREA_RATIO:
                ...
                continue
            if (
                width_ratio > CUSTOM_MAX_BOX_WIDTH_RATIO
                or height_ratio > CUSTOM_MAX_BOX_HEIGHT_RATIO
            ):
                logging.info(
        ...
        if (
            detector == "coco"
            and category == "animal"
            and confidence < COCO_MIN_ANIMAL_CONFIDENCE
        ):
            logging.info(
        ...
        if (
            detector == "coco"
            and category == "object"
            and confidence < COCO_MIN_OBJECT_CONFIDENCE
        ):
            logging.info(
...
def estimate_detection_guidance(
    best: Dict[str, Any], payload: Dict[str, Any]
) -> Dict[str, Any]:
...
def estimate_map_position(guidance: Dict[str, Any]) -> Dict[str, float]:
    ...
    return {
        "x": max(
            0.0,
            min(
                800.0, base.rover_pose["x"] + forward_x * forward_cm + right_x * side_cm
            ),
        ),
        "y": max(
            0.0,
            min(
                800.0, base.rover_pose["y"] + forward_y * forward_cm + right_y * side_cm
            ),
        ),
    }
...
def fallback_detection_result(payload: Dict[str, Any]) -> Dict[str, Any]:
    detections = [
        item for item in payload.get("detections", []) if isinstance(item, dict)
    ]
    for category in ("human", "animal", "object", "marker", "fire", "track"):
        ...
        if matches:
            ...
            object_phrase = (
                "person" if category == "human" and label == "person" else label
            )
            return {
                "detected": True,
...
def build_detection_notes(payload: Dict[str, Any]) -> str:
    ...
    for detection in detections[:8]:
        ...
        lines.append(
            f"- label={label}, category_hint={category_hint}, confidence={confidence}, box={box}"
        )
...
def review_object_detection(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if provider == "ollama":
            ...
            logging.warning(
                "Unknown object review provider %r; using fallback.", provider
            )
            return fallback_detection_result(payload)
        return base.parse_json_response(text)
    except Exception as exc:
        logging.warning(
            "Local object review model failed; using fallback detection result: %s", exc
        )
        return fallback_detection_result(payload)
...
def emit_dashboard_alert(result: Dict[str, Any]) -> None:
    ...
    base.socketio.emit(
        "rover_status",
        {
            "status": "vision_detected",
            "message": f"{alert_payload['category']}: {alert_payload['message']}",
        },
    )
...
def windows_object_detection_loop() -> None:
    ...
    detectors = []
    if WINDOWS_DETECTOR_RUNTIME in {"ultralytics", "yoloe", "yolo"}:
        ultralytics_detector = UltralyticsDetector(
    ...
    if not detectors:
        logging.warning(
            "Windows-side detection is inactive. Check model/runtime/dependencies."
        )
        return
    ...
    while True:
        ...
        try:
            ...
            detections = filter_scene_detections(
                detections, working_frame.shape[1], working_frame.shape[0]
            )
            detections.sort(key=confidence_score, reverse=True)
            detections = detections[:ONNX_MAX_DETECTIONS]
            postprocessed = detection_postprocessor.process(
                detections, working_frame.shape[1], working_frame.shape[0]
            )
            for rejected in postprocessed["rejected"]:
                base.record_detection_event(
                    rejected, False, str(rejected.get("reason") or "rejected")
                )
            for candidate in postprocessed["candidates"]:
                base.record_detection_event(
                    candidate, False, "insufficient_persistence"
                )
            ...
            visible_detections = [
                *postprocessed["candidates"],
                *postprocessed["confirmed"],
            ]
...
def handle_rover_report(payload: Any) -> None:
    ...
    if isinstance(payload, dict) and payload.get("type") == "object_detection":
        if DETECTION_AUTHORITY in {"pi", "rover", "raspberry_pi", "raspberry-pi"}:
            ...
            return
        logging.info(
            "Ignoring Pi-side object_detection payload because Windows YOLO is authoritative."
        )
        return
    original_handle_rover_report(payload)
...
def main() -> None:
    ...
    if DETECTION_AUTHORITY in {"pi", "rover", "raspberry_pi", "raspberry-pi"}:
        logging.info(
            "Pi-side detection authority enabled; Windows YOLO analysis loop is disabled."
        )
    else:
