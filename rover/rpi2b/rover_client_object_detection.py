...
class ObjectDetectionConfig:
    ...
    common_target_classes: list[str] = field(
        default_factory=lambda: ["person", "cell phone"]
    )
    ignored_classes: list[str] = field(
        default_factory=lambda: sorted(BACKGROUND_LABELS)
    )
...
def should_keep_detection(
    ...
    if label in ignored:
        return False
    area_percent = (
        float(box.get("width", 0)) * float(box.get("height", 0)) * 100.0
    ) / max(1.0, float(frame_width * frame_height))
    return (
        float(config.min_box_area_percent)
        <= area_percent
        <= float(config.max_box_area_percent)
    )
...
class CocoObjectDetector:
    ...
    def setup(self) -> bool:
        ...
        missing = [
            str(path)
            for path in (names_path, config_path, weights_path)
            if not path.exists()
        ]
        if missing:
            logging.warning(
                "Object detector model files missing: %s", ", ".join(missing)
            )
            return False
        ...
        logging.info(
            "Pi-side COCO object detector loaded with %d classes.",
    ...
    def detect(self, frame: Any) -> list[dict[str, Any]]:
        ...
        for class_id, confidence, box in zip(
            ...
            if not should_keep_detection(
                label_lower, bbox, int(frame.shape[1]), int(frame.shape[0]), self.config
            ):
                continue
            category_hint = category_for_label(
                label_lower, allow_general_objects=not bool(target_classes)
            )
...
class YoloObjectDetector:
    def __init__(self, config: ObjectDetectionConfig, root: Path) -> None:
        ...
        self.model_source = ""
        self.is_promptable_open_vocab = False
    ...
    def setup(self) -> bool:
        ...
        configured = str(self.config.yolo_model_path or "").strip()
        if not configured:
            logging.warning("YOLO model path/name is empty.")
            return False

        model_path = self._resolve(configured)
        self.model_source = str(model_path)

        if (
            model_path.is_dir()
            and (model_path / "model.ncnn.param").exists()
            and (model_path / "model.ncnn.bin").exists()
        ):
            self.model_source = str(model_path)
            return self._setup_ncnn(model_path)

        looks_like_local_path = any(sep in configured for sep in ("/", "\\"))
        if not model_path.exists() and looks_like_local_path:
        ...
        if model_path.exists():
            ...
            self.model_source = configured
            logging.info(
                "YOLO model %s not found locally; ultralytics will try to download it.",
                configured,
            )
        ...
        self.is_promptable_open_vocab = "yoloe" in self.model_source.lower()
        if self.is_promptable_open_vocab and YOLOE is not None:
            self.model = YOLOE(self.model_source)
        else:
            self.model = YOLO(self.model_source)

        target_classes = [
            label_key(item)
            for item in (self.config.target_classes or [])
            if str(item).strip()
        ]
        if self.is_promptable_open_vocab and target_classes:
            if hasattr(self.model, "set_classes"):
                self.model.set_classes(target_classes)
                logging.info("Configured YOLOE text prompts: %s", target_classes)
            else:
                logging.warning(
                    "Model %s looks like YOLOE but does not expose set_classes().",
                    self.model_source,
                )
        elif self.is_promptable_open_vocab and YOLOE is None:
            logging.warning(
                "YOLOE model requested (%s) but installed ultralytics does not expose YOLOE class. "
                "Upgrade ultralytics on the Pi.",
                self.model_source,
            )

        names = {
            str(value).lower() for value in getattr(self.model, "names", {}).values()
        }
        if names and "person" not in names and not self.is_promptable_open_vocab:
            logging.warning(
        ...
        logging.info("Pi-side YOLO object detector loaded from %s.", self.model_source)
        return True
    ...
    def _setup_ncnn(self, model_path: Path) -> bool:
        try:
            ...
            logging.error(
                "ncnn is required for direct NCNN model loading. Install with: python -m pip install ncnn"
            )
            return False
        ...
        logging.info(
            "Pi-side NCNN YOLO detector loaded from %s with %d classes.",
            model_path,
            len(self.ncnn_names),
        )
        return True
    ...
    def _detect_ncnn(self, frame: Any) -> list[dict[str, Any]]:
        ...
        for row in raw:
            ...
            if not should_keep_detection(
                label, bbox, frame_width, frame_height, self.config
            ):
                continue
        ...
        kept = cv2.dnn.NMSBoxes(
            boxes, scores, threshold, self._ratio(self.config.nms_threshold)
        )
        kept_indices = [int(i) for i in np.array(kept).flatten()] if len(kept) else []
        detections: list[dict[str, Any]] = []
        for index in kept_indices[:12]:
            label = label_key(
                self.ncnn_names.get(class_ids[index], str(class_ids[index]))
            )
    ...
    def detect(self, frame: Any) -> list[dict[str, Any]]:
        ...
        for box in boxes:
            ...
            if not should_keep_detection(
                label, bbox, int(frame.shape[1]), int(frame.shape[0]), self.config
            ):
                continue
            category_hint = category_for_label(label, allow_general_objects=True)
            if not category_hint:
...
class CombinedObjectDetector:
    ...
    def setup(self) -> bool:
        ...
        if self.config.common_detection_enabled:
            ...
            common_config.target_classes = list(
                self.config.common_target_classes or ["person", "cell phone"]
            )
        ...
        if custom_ready and common_ready:
            logging.info(
                "Combined detector ready: custom YOLO plus common person/cell-phone COCO."
            )
        elif custom_ready:
            logging.warning(
                "Combined detector running custom YOLO only; common COCO detector unavailable."
            )
        elif common_ready:
            logging.warning(
                "Combined detector running common COCO only; custom YOLO detector unavailable."
            )
    ...
    def detect(self, frame: Any) -> list[dict[str, Any]]:
        ...
        common_every = max(
            1, int(getattr(self.config, "common_detection_every_n", 1) or 1)
        )
            ...
            detections.append(
                {
                    ...
                    "confidence": min(
                        ...
                        round(
                            max(
                                fill_ratio,
                                area / max(1.0, frame.shape[0] * frame.shape[1] * 0.08),
                            ),
                            3,
                        ),
                    ),
...
class ObjectDetectionRoverClient(base.RoverClient):
    def __init__(
        self,
        config: base.RoverConfig,
        hardware: base.HardwareBase,
        detector_config: ObjectDetectionConfig,
        root: Path,
    ) -> None:
    ...
    def run(self) -> None:
        ...
        threading.Thread(
            target=self.detection_loop, name="pi-detection-loop", daemon=True
        ).start()
        super().run()

    def _send_object_detection(
        self, detections: list[dict[str, Any]], frame: Any
    ) -> None:
        now = time.monotonic()
        signature = ",".join(
            f"{item['label']}:{item['category_hint']}" for item in detections[:4]
        )
    ...
    def _overlay_detections(self, frame: Any) -> Any:
        ...
        for item in detections[:12]:
            ...
            color = (
                (0, 255, 0) if item.get("category_hint") != "marker" else (80, 255, 80)
            )
    ...
    def camera_loop(self) -> None:
        ...
        try:
            ...
            logging.error(
                "opencv-python is required for Pi webcam streaming and object detection."
            )
            return
        ...
        while not self.stop_requested.is_set():
            ...
            logging.info(
                "Streaming Pi webcam smoothly at %sx%s; object detection runs on a slower side interval.",
                camera.width,
                camera.height,
            )
            ...
            while not self.stop_requested.is_set():
                ...
                if self.connected.is_set():
                    ...
                    stream_interval = max(
                        0.05,
                        float(
                            self.detector_config.stream_interval_seconds
                            or camera.upload_interval_seconds
                        ),
                    )
                    if now - last_stream_sent >= stream_interval:
                        display_frame = self._overlay_detections(frame)
                        encoded_ok, encoded = self._encode_frame(
                            display_frame, self.detector_config.stream_jpeg_quality
                        )
                        if encoded_ok:
                            self.send_json(
                                {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="STASIS Raspberry Pi rover client with Pi-side object detection"
    )
...
def main() -> None:
    ...
    if not config.server_host:
        raise SystemExit(
            "STASIS server host ip is unspecified. Use config.json or --server <IP>."
        )

    hardware: base.HardwareBase = (
        base.SimulatedHardware(config)
        if config.simulate
        else base.RealRoverHardware(config)
    )
    detector_config = load_object_detection_config(args.config)
    client = ObjectDetectionRoverClient(
        config, hardware, detector_config, Path(__file__).resolve().parent
    )
