...
class MotionConfig:
    ...
    turn_tolerance_deg: float = (
        5.0  # Acceptable angular error margin to declare a turn complete
    )
    turn_min_pwm: float = (
        35.0  # Minimum duty cycle to overcome static friction during turning
    )
    turn_max_pwm: float = (
        85.0  # Cap on motor speed duty cycle during high-error PID turns
    )
    drive_pwm: float = 70.0  # Default forward driving speed duty cycle
    drive_ms_per_cm: float = (
        50.0  # Open-loop calibration constant mapping drive duration to distance
    )
    turn_timeout_seconds: float = (
        12.0  # Timeout limit before aborting a closed-loop turn operation
    )
    telemetry_interval_seconds: float = (
        0.5  # Periodicity of sending status packets back to the dashboard
    )
    ...
    scan_settle_seconds: float = (
        0.25  # Delay to let servo stabilize before firing ultrasound trigger
    )
...
class HardwareConfig:
    ...
    direct_motor_control: bool = (
        True  # Directly drive motor controller from Raspberry Pi GPIO pins
    )
    motor_driver: str = "l911s"  # Direct GPIO driver type: l911s or l298n
    esp32_serial_control: bool = (
        False  # Delegate L911S motor driving to ESP32-S3 over Serial
    )
    ...
    require_heading_for_goto: bool = (
        False  # Reject navigation goals if no active IMU/Compass exists
    )
    open_loop_turn_degrees_per_second: float = (
        90.0  # Fallback calibration constant for non-IMU turns
    )
    obstacle_stop_distance_cm: float = (
        25.0  # Minimum distance threshold before emergency stopping
    )
    obstacle_check_interval_seconds: float = (
        0.1  # Polling rate of ultrasonic sensor during drive forward
    )
...
class RoverConfig:
    ...
    def from_mapping(cls, data: dict[str, Any]) -> RoverConfig:
        ...
        if "hardware" in values:
            values["hardware"] = _dataclass_from_mapping(
                HardwareConfig, values["hardware"]
            )
        if "camera" in values:
...
def read_config(path: Path | None) -> RoverConfig:
    ...
    try:
        if path is not None and path.exists():
            config = RoverConfig.from_mapping(
...
class SimulatedHardware(HardwareBase):
    ...
    def setup(self) -> None:
        logging.info(
            "Simulation mode enabled; no GPIO, I2C, or Serial hardware will be used"
        )
...
class MotorDriver:
    ...
    def setup(self) -> None:
        ...
        if self.driver_type not in {"l911s", "l298n"}:
            raise RuntimeError(
                f"Unsupported direct motor driver type: {self.driver_type}"
            )

        logging.info(
            "Configuring direct Raspberry Pi GPIO motor pins for %s...",
        ...
        try:
            ...
            enable_pins = tuple(
                pin
                for pin in (self.pins.left_motor_enable, self.pins.right_motor_enable)
                if pin is not None
            )
            ...
            logging.exception(
                "Failed to configure RPi.GPIO pins for direct motor control"
            )
    ...
    def set_speeds(self, left_speed: float, right_speed: float) -> None:
        ...
        if self.driver_type == "l298n":
            ...
            self._set_pair(
                self.pins.left_motor_forward, self.pins.left_motor_reverse, left_speed
            )
            self._set_pair(
                self.pins.right_motor_forward,
                self.pins.right_motor_reverse,
...
class SerialMotorDriver:
    ...
    def setup(self) -> None:
        ...
        logging.info(
            "Connecting to ESP32 motor controller at %s (%d baud)...",
            self.port,
            self.baudrate,
        )
        try:
            ...
            raise RuntimeError(
                f"Could not connect to ESP32 motor controller on {self.port}. Verify port and permissions."
            ) from e
                    ...
                    for line in raw_data.splitlines():
                        ...
                        if line:
                            ...
                            if "WARNING" in line or "ERR" in line:
                                logging.warning(
                                    "ESP32 Warning/Error reported: %s", line
                                )
            except Exception as exc:
...
class MPU6050:
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            self.bus.write_byte_data(
                self.address, self.GYRO_CONFIG, 0x08
            )  # +/- 500 deg/s
            self.bus.write_byte_data(
                self.address, self.CONFIG, 0x04
            )  # 21 Hz low-pass filter
            time.sleep(0.1)
            self.calibrate()
            self.ready = True
    ...
    def read_gyro_z_deg_per_sec(self) -> float:
        ...
        """
        return (
            self.read_s16(self.GYRO_ZOUT_H) / 65.5
        )  # Sensitivity scale factor for +/- 500 deg/s
    ...
    def calibrate(self) -> None:
        ...
        logging.info(
            "Calibrating MPU6050 gyro (sampling %d cycles)... Keep rover still.",
            samples,
        )
        total = 0.0
        ...
        self.last_update = time.monotonic()
        logging.info(
            "Gyro Z Calibration completed. Bias offset: %.3f deg/s", self.gyro_z_bias
        )
...
class HMC5883L:
    ...
    def setup(self) -> None:
        ...
        try:
            self.bus.write_byte_data(
                self.address, 0x00, 0x70
            )  # 8-sample avg, 15 Hz default
            self.bus.write_byte_data(self.address, 0x01, 0x20)  # Gain setting
            self.bus.write_byte_data(
                self.address, 0x02, 0x00
...
class ServoScanner:
    ...
    def __init__(
        self, gpio: Any, servo_pin: int, sensor: UltrasonicSensor, motion: MotionConfig
    ) -> None:
        self.gpio = gpio
    ...
    def write_angle(self, angle: int) -> None:
        ...
        duty_cycle = (
            2.5 + angle / 18.0
        )  # standard mapping to 0.5ms - 2.5ms pulse widths
        try:
            self.pwm.ChangeDutyCycle(duty_cycle)
            time.sleep(0.05)
            self.pwm.ChangeDutyCycle(
                0
            )  # Stop sending signal pulse to avoid servo jitter
        except Exception as exc:
    ...
    def scan(self) -> list[dict[str, float]]:
        ...
        for angle in range(0, 181, self.motion.scan_step_deg):
            ...
            readings.append(
                {"angle": float(angle), "distance": self.sensor.distance_cm()}
            )
...
class RealRoverHardware(HardwareBase):
    ...
    def setup(self) -> None:
        ...
        if self.config.hardware.esp32_serial_control:
            ...
            if GPIO is None:
                logging.warning(
                    "Direct motor control requested, but GPIO is unavailable; continuing without motor output."
                )
            else:
                self.motors = MotorDriver(
                    GPIO, self.config.pins, self.config.hardware.motor_driver
                )
                try:
                    self.motors.setup()
                except Exception:
            ...
            logging.warning(
                "Motor drivers disabled in configuration; speeds will be ignored."
            )
    ...
    def _setup_i2c(self) -> None:
        ...
        try:
            ...
            logging.warning(
                "smbus2 is not installed; MPU6050 and HMC5883L compass disabled."
            )
            return
        ...
        if self.config.hardware.mpu6050_enabled:
            self.imu = MPU6050(
                self.bus, self.config.sensors.mpu6050_address, self.config.sensors
            )
            try:
                self.imu.setup()
            except OSError:
...
class RoverClient:
    ...
    def on_open(self, ws: websocket.WebSocketApp) -> None:
        logging.info(
            "WebSocket handshake successful. Connected to %s", self.websocket_url
        )
        self.connected.set()
    ...
    def on_close(
        self, ws: websocket.WebSocketApp, status_code: int, message: str
    ) -> None:
    ...
    def camera_loop(self) -> None:
        ...
        try:
            ...
            logging.error(
                "opencv-python is required for Pi webcam streaming. Install rover/rpi2b requirements."
            )
            return
    ...
    def handle_vision_result(self, payload: dict[str, Any]) -> None:
        ...
        if category == "human":
            ...
            if self.get_mode() == MODE_FOLLOW:
                ...
                self.set_mode(
                    MODE_WAITING_DECISION,
                    "human detected; waiting for dashboard decision",
                )
        elif category == "fire":
            self.movement_cancel.set()
            self.hardware.stop_motors()
            action = "stop_and_alert"
            self.set_mode(
                MODE_WAITING_DECISION, "fire detected; waiting for dashboard decision"
            )
    ...
    def start_follow_mode(self) -> None:
        ...
        if self.follow_thread is None or not self.follow_thread.is_alive():
            self.follow_thread = threading.Thread(
                target=self.follow_loop, name="follow-loop", daemon=True
            )
    ...
    def follow_loop(self) -> None:
        ...
        while not self.stop_requested.is_set() and self.get_mode() == MODE_FOLLOW \
            ...
            frame_height = float(
                target.get("frame_height") or target.get("height") or 480
            )
            ...
            if (
                ...
                self.hardware.stop_motors()
                self.set_mode(
                    MODE_EMERGENCY_STOP, f"Obstacle at {front_distance:.0f} cm"
                )
                break
            ...
            forward = (
                motion.follow_base_pwm
                if area_ratio < motion.follow_target_area_ratio
                else 0.0
            )
            turn = (
                0.0
                if abs(center_error) < motion.follow_deadband_ratio
                else center_error * motion.follow_turn_pwm
            )
            left_speed = max(
                -motion.turn_max_pwm, min(motion.turn_max_pwm, forward + turn)
            )
            right_speed = max(
                -motion.turn_max_pwm, min(motion.turn_max_pwm, forward - turn)
            )
    ...
    def handle_goto(self, angle: float, distance_cm: float) -> None:
        ...
        if heading is None:
            ...
            logging.warning(
                "Compass/IMU readings absent. Executing timed open-loop alignment."
            )
            self.open_loop_turn_to_angle(angle)
        else:
            self.last_heading = heading
            logging.info(
                "Executing closed-loop PID alignment to target: %.1f deg", angle
            )
    ...
    def turn_to_angle(self, target_angle: float) -> None:
        ...
        while (
            ...
            if heading is None:
                logging.error(
                    "Lost heading feedback midpoint of turning. Aborting turn."
                )
                break
            ...
            if abs(error) <= motion.turn_tolerance_deg:
                logging.info(
                    "PID turning target reached. Margin error: %.2f deg.", error
                )
                break
            ...
            speed = max(
                motion.turn_min_pwm,
                min(motion.turn_max_pwm, abs(error) * motion.turn_kp),
            )
            if error > 0:
    ...
    def open_loop_turn_to_angle(self, target_angle: float) -> None:
        ...
        speed = max(
            motion.turn_min_pwm, min(motion.turn_max_pwm, abs(error) * motion.turn_kp)
        )
        if error > 0:
    ...
    def drive_forward(self, distance_cm: float) -> str:
        ...
        while (
            ...
            if (
                now - last_obstacle_check
                >= self.config.hardware.obstacle_check_interval_seconds
            ):
            ...
            progress = (
                min(1.0, (time.monotonic() - start) / duration) if duration > 0 else 1.0
            )
            self.distance_traveled_cm = start_distance + distance_cm * progress
            self.hardware.set_motors(
                self.config.motion.drive_pwm, self.config.motion.drive_pwm
            )
            time.sleep(0.01)
    ...
    def request_stop(self, *_args: Any) -> None:
        ...
        """
        logging.info(
            "RoverClient termination request received. Safely stopping motors."
        )
    ...
    def run(self) -> None:
        ...
        threading.Thread(
            target=self.command_loop, name="command-loop", daemon=True
        ).start()
        threading.Thread(
            target=self.telemetry_loop, name="telemetry-loop", daemon=True
        ).start()
        threading.Thread(
            target=self.camera_loop, name="pi-camera-loop", daemon=True
        ).start()

        # Connect / reconnect loop
        while not self.stop_requested.is_set():
            logging.info(
                "Initiating connection to server %s as %s",
                self.websocket_url,
                self.config.rover_id,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--server", help="Laptop/server IP or hostname running Flask on port 5000"
    )
    ...
    parser.add_argument(
        "--simulate", action="store_true", help="Run without physical sensor interfaces"
    )
...
def main() -> None:
    ...
    if not config.server_host:
        raise SystemExit(
            "STASIS server host ip is unspecified. Use config.json or specify via --server <IP>."
        )

    # Select hardware implementation
    hardware: HardwareBase = (
        SimulatedHardware(config) if config.simulate else RealRoverHardware(config)
    )
    ...
    try:
        ...
        logging.critical(
            "Stasis rover client terminated with a fatal exception: %s", exc
        )
    finally:
